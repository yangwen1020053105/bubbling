...
(function (layui, window, factory) {
	...
	Common.prototype.render = function (id, select) {
		...
		$(".xm-select").each(function(){
		   var _this = $(this);
		   var observer = new MutationObserver(function(mutations) {//构造函数回调
		      mutations.forEach(function(record) { 
		      if($(_this[0]).parent().parent().parent().prev().prev().attr("class")!=null&&$(_this[0]).parent().parent().parent().prev().prev().attr("class").indexOf("xmselect")!=-1){
		            $(_this[0]).parent().parent().parent().prev().prev().val(getSelectVal($(_this[0]).parent().parent().parent().prev().attr("xm-select"))) 
		            $(_this[0]).parent().parent().parent().prev().prev().change();   
		      }　　　  
		      });
		   });
		...
		$(".xmselect").each(function(){
		   ...
		   var observer = new MutationObserver(function(mutations) {//构造函数回调
		      mutations.forEach(function(record) { 
		      　　　  if($(_this[0]).next().attr("xm-select")!=null&&$(_this[0]).next().attr("xm-select")!=""){
			     setSelectVal($(_this[0]).next().attr("xm-select"),$(_this[0]).val());
			   }
		      });
		   });
		   observer.observe(_this[0], config);
